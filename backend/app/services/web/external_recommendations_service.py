...
from app.config import RECOMMENDER_ENDPOINT, RECOMMENDATION_TYPES
...
class ExternalRecommendationsService:
    @staticmethod
    async def fetch(
            client: AsyncClient, session: SessionData | None, type: RECOMMENDATION_TYPES
    ):
        uuids = await ExternalRecommendationsService._get_recommended_uuids(client, session, type)
        return await ExternalRecommendationsService._get_recommended_items(client, uuids)

    @staticmethod
    def _get_panel(panel_id: RECOMMENDATION_TYPES) -> list[str]:
        match panel_id:
            case "publication":
                return ["publications"]
            case "dataset":
            ...
            case "training":
                return ["trainings"]
        raise ValueError(f"{panel_id} is not valid {RECOMMENDATION_TYPES}")

    @staticmethod
    async def _get_recommended_uuids(
            client: AsyncClient, session: SessionData | None, panel_id: RECOMMENDATION_TYPES
    ):
        try:
